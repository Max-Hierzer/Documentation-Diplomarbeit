...
\section{MyPolls}
\subsection{Purpose}
The "MyPolls" section provides access to all polls a user created, they share links for those polls and also the option to delete them. It allows users to manage their polls from one central location.
\subsection{Polllink}
The Polllink for sharing the Polls is created by hashing key information with the "btoa" algorithm. In order to make the hash browser navigation compatible it is also encoded as an URI component with the same name(encodeURIComponent). The hash contains information about if the poll is public, if it is anonymous, about the mode and the poll's ID.\\\\
When opening the Polllink the hash gets URI and "atob" decoded and depending on the mode the corresponding section in the dashboard and the poll with the right ID gets opened. Should the ID not exist in this specific mode, or in general, it just opens the right mode in the default state.\\\\
