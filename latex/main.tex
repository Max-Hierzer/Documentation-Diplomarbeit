...
% Custom JSX code box with black border
\newtcblisting{jsx}{
	listing engine=minted,
	listing only,
	minted language=jsx,
...
In response to client requirements, a feature was implemented allowing users to upload a title image for each poll. This image serves a purely aesthetic purpose, aiming to enhance the visual appeal of the poll and provide users with a more engaging and intuitive introduction to the survey topic. By incorporating visual context, the feature helps draw user attention and encourages participation, particularly in cases where the subject matter benefits from visual reinforcement.\\\\
In this project, images are stored on the server's local filesystem and served statically through Express. While this approach is sufficient for development and small-scale use, a dedicated static file server would be more appropriate for a production environment due to better scalability and performance.
\subsubsection{Backend Implementation}
On the backend, the image upload is handled using the "multer" middleware in \texttt{imageRoutes.js}. The storage engine is configured to save uploaded files in the \texttt{uploads} directory with a unique filename based on a timestamp and the original file extension. The file types are restricted to JPEG, PNG and GIF formats through a custom file filter to ensure consistency and security. \parencite{expressmulter}
\begin{figure}[H]
	\begin{code}
...
\begin{enumerate}
	...
	\item To prepare the image for upload to the server, a new instance of the \textit{FormData} object is created. This object is specifically designed to handle form submissions that include binary data such as files. The use of \textit{FormData} is necessary because standard JSON-based payloads \texttt{application/json} do not support file transfers. While an alternative approach would be to encode images as Base64 strings and send them in a JSON payload, this method increases payload size and adds decoding overhead. For efficiency and compatibility with file upload workflows \texttt{multipart/form-data} via \textit{FormData} is the preferred choice in this project. \parencite{avoid-base64}
...
To allow for the further analysis and documentation of the poll results, a CSV export was implemented. This export privides a structured overview of all user responses, including additional metrics, such as the number of votes per answer and the average weight for weighted questions.\\ \\
The logic is handled in \texttt{csvExportController.js}. Upon receiving a request, the system retrieves all relevant data for a given poll by its ID, including associated questions, answers, and question types. The raw voting data is processed to count the number of responses per answer and to calculate the average weight if applicable. Special care is taken to distinguish between different types: for instance, \texttt{Single Choice} questions ensure that only one answer per user is counted, while \texttt{Weighted Choice} questions include the weight value in the computation.\\ \\
The processed data is then converted into a structured CSV format using the json2csv library. The final file includes columns such as Poll Name, Question, Question Type, Answer, Vote Count and Average Weight, and can be downloaded directly by the user. This feature ensures transparency and enables further statistical evaluation using external spreadsheet or data analysis tools. \parencite{json2csv} \\ \\
...
\subsection{Tooltips}
To enhance the usability and user experience of the application, tooltips were attached to all input fields, providing users with contextual guidance and reducing the likelihood of input errors. For this purpose, the React library "react-tooltips" was utilized, offering a high degree of customizability in both design and behavior. This allowed for the implementation of consistent and informative tooltip elements throughout the interface. \parencite{tooltips} \\ \\
Each element is assigned a unique \textit{data-tool-id}, which serves as a reference key within the central \texttt{Tooltips.js} configuration file. This file defines the content, styling, and interaction logic for each tooltip, ensuring that the visual presentation and timing behavior are consistent across the application. To prevent the tooltips from becoming distracting during regular use, a delay of 1500 milliseconds was introduced before displaying the tooltip on hover.\\ \\
...
At the beginning of the development process, a rough design was implemented to help visualize the overall concept of the future application. In order to maintain a clean and organized project structure, general styling files such as \texttt{App.css}, capitalized to match the App component, \texttt{dashboard.css}, \texttt{voting.css}, and others were placed within a dedicated styles directory.\\\\
...
To ensure consistency of the palette, user defined characteristics from CSS were used to assign variables for each main color. \parencite{csscolorvariables}
\begin{figure}[h!]
	\begin{csscode}
		:root {
...
\end{figure}
Those variables were then used in the appropriate element. Therefore changing the color scheme to a different one is less afford for the developers or maintainers. \\ \\
The basic structure of the page is split into three sections: header with navigation, main with content and footer with imprint and privacy policy. The default style is based on the client's corporate design. Therefore, the standard background is purple but the content background is still white to ensure readability. \parencite{lmppage} \\
...
To ensure responsiveness in each section of the application, \textbf{@media CSS at-rule} was used to apply different styles based on the screen size or device characteristics. This approach allows the layout, font sizes and element spacing to adapt dynamically, creating a smoother experience for users on any device. The navigation bar was designed with media queries to transform into so-called "burger menu" when the screen width falls below a certain threshold. This ensures usability on smaller screens.
\begin{figure}[h!]
	\centering
