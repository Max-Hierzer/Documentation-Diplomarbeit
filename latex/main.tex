...
The Create Polls section depicts the poll creation interface of the system. It allows users to create customized polls based on their needs. Customization options include a poll name and description, setting different poll types, adding the poll to specific groups, a start and end date for the voting period, an image upload, a variable amount of questions with chosen question types and a variable amount of answers for each question.
\subsection{Validation}
To ensure a correct poll creation the Create Polls features validation in the frontend and also the backend.
\subsubsection{Frontend}
In the frontend the validation works through a separate file, \textbf{ValidatePolls.js}. It checks if any input field is left blank.
\begin{figure}
	\begin{code}
...
\section{MyPolls}
\subsection{Purpose}
The "MyPolls" section central location where users can view and manage all polls they have created. It provides access to the links for the polls as well as a delete option for them.
